...
**BẮT BUỘC trước mỗi lần commit**: đối chiếu `brainstorming/master-plan.md` với hiện trạng thật (code vừa đổi, phase vừa xong/đang làm) — nếu lệch, cập nhật file trước, rồi mới commit. Không commit khi master-plan chưa phản ánh đúng thực tế.

Khi giải quyết xong 1 bug không hiển nhiên (nguyên nhân mất công mới tìm ra) hoặc xác nhận được 1 cách tiếp cận đúng/sai qua thực tế, ghi vào `brainstorming/findings.md` — mục đích để không phải dò lại từ đầu khi gặp vấn đề tương tự sau này. Khi gặp lỗi lạ, tra `findings.md` trước khi debug lại từ đầu.
...
- **Khi thiết kế tính năng mới: luôn cân nhắc dùng AI ở những chỗ có thể** để sản phẩm thông minh/dynamic hơn (ví dụ: cá nhân hoá sâu hơn, tự động phân loại, gợi ý thông minh thay vì rule tĩnh) — nhưng đây là phương châm CÂN NHẮC, không phải mặc định áp dụng. Luôn đề xuất + **hỏi xác nhận user trước** khi quyết định đưa AI vào một chỗ cụ thể, không tự ý thêm.
...
- 🔄 `brainstorming/findings.md` — **bug/gotcha kỹ thuật đã gặp + cách tiếp cận đã kiểm chứng**, cập nhật liên tục; tra trước khi debug lại từ đầu 1 vấn đề có thể đã từng gặp
